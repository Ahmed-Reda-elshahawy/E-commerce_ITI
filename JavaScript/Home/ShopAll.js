...
function DisplayAllProducts() {
    ...
    if (products) {
        ...
        const productsPerPage = 18;
        let currentPage = parseInt(localStorage.getItem("currentPage")) || 1; // Restore the current page
        ...
        const renderProducts = (filteredProducts, page) => {
            ...
            // Attach event listeners to the newly rendered product cards
            const productCards = document.querySelectorAll(".product-card");
            productCards.forEach(card => {
                card.addEventListener("click", function () {
                    const productId = card.getAttribute("data-product-id");
        ...
        // Initial render (restore the saved page)
        renderProducts(shuffledProducts, currentPage);
